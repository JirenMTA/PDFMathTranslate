...
if __name__ == "__main__":
    test_file_path = "./test_file/unet_model.pdf"

    doc_raw = open(test_file_path, 'rb')
    s_raw = doc_raw.read()
    stream = io.BytesIO()
    doc_raw.close()
    ...
    for page_no, page in enumerate(PDFPage.create_pages(doc)):
        pix = doc_en[page_no].get_pixmap()
        image = np.frombuffer(pix.samples, dtype=np.uint8).reshape(pix.height, pix.width, 3)[:, :, ::-1]
        img_rgb = image.copy()
        page_layout = model.predict(image, imgsz=int(pix.height / 32) * 32)[0]

        for idx, box in enumerate(page_layout.boxes):
            x0, y0, x1, y1 = map(int, box.xyxy.squeeze())
            cv2.rectangle(img_rgb, (x0, y0), (x1, y1), (0, 255, 0), 2)

        plt.figure(figsize=(10, 14))  # tweak size to taste
